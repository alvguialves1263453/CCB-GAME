class SoundService {
  ...
  playBuzzer() {
    try {
      ...
      const t = this.ctx.currentTime;
      // 3 toques de "acabou o tempo!" (corneta)
      [0, 0.35, 0.7].forEach((off) => {
        ...
        gainNode.gain.setValueAtTime(0, t + off);
        gainNode.gain.linearRampToValueAtTime(0.18, t + off + 0.03);
        gainNode.gain.exponentialRampToValueAtTime(0.01, t + off + 0.3);
